export const TOOL_PROMPTS = {
  subagent: {
    ...
    parameterDescriptions: {
      ...
      model: "Optional model ID or provider/model ID for this call. Usually only for the default agent; named agents pin their model in frontmatter.",
      thinkingLevel: "Optional thinking level for this call: one of `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Usually only for the default agent; must be supported by the resolved model or the run fails listing the supported levels.",
    ...
    promptGuidelines: [
      "Common calls: subagent({ task: \"...\" }) for independent work, subagent({ model: \"acme/acme-5v-turbo\", task: \"Read screenshots/login.png and return its contents\" }) to pick a default-mode child's model, or subagent({ context: \"fork\", task: \"...\" }) when the child should inherit a copy of the current conversation.",
      ...
      "Call-level `model`/`thinkingLevel` are usually only for the default agent; named agents pin their own in frontmatter.",
      "An explicit `thinkingLevel` must be supported by the resolved child model, or the run fails listing the supported levels.",
